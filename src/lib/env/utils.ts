...
import type { KoosEnv, KoosEnvAny, KoosEnvBoolean, KoosEnvInteger, KoosEnvString } from "./types.js";

export function envParseInteger(key: KoosEnvInteger, defaultValue?: number): number {
...
export function envParseBoolean(key: KoosEnvBoolean, defaultValue?: boolean): boolean {
    const value = process.env[key];
    if (isNullishOrEmpty(value)) {
...
export function envParseString<K extends KoosEnvString>(key: K, defaultValue?: KoosEnv[K]): KoosEnv[K] {
    const value = process.env[key];
    if (isNullishOrEmpty(value)) {
...
export function envParseArray(key: KoosEnvString, defaultValue?: string[]): string[] {
    const value = process.env[key];
    if (isNullishOrEmpty(value)) {
...
export function envIsDefined(...keys: readonly KoosEnvAny[]): boolean {
